...
pkgs.runCommand "balaur-dashboard-tests"
  ...
  ''
    ...
    grep --fixed-strings 'serviceUrl(8084, "/", "https:")' index.html
    ...
    test "$(grep --only-matching 'class="service-icon"' index.html | wc --lines)" = 6
    ...
    grep --fixed-strings 'data.disks.map' index.html
      ...
      jq --exit-status '
        ...
        and ([.services[].id] == ["home-assistant", "jellyfin", "prowlarr", "qbittorrent", "trilium", "fastflowlm"])
